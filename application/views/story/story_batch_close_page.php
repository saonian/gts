    ...
    <table class="table-1 fixed">
      ...
      <tbody>
        ...
        <tr class="a-center">
          ...
          <td><?php echo $pmsdata['story']['status'][$val->status]['display'];?></td>
          <td><select name="closed_reason[]" <?php echo $val->status == $pmsdata['story']['status']['closed']['value']?'disabled="true"':'';?>>
              <?php foreach ($pmsdata['story']['close_reason'] as $v):?>
                <option value="<?php echo $v['value'];?>"><?php echo $v['display'];?></option>
              <?php endforeach;?>
              </select></td>
          <td class="a-left"><input name="comment[]" value="" class="text-1" <?php echo $val->status == $pmsdata['story']['status']['closed']['value']?'disabled="true"':'';?>/></td>
        </tr>
        <?php endforeach;?>
        <tr>
