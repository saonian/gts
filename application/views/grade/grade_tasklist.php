        ...
        <form id="projectStoryForm" method="post">
            <table class="table-1 colored tablesorter datatable">
                <caption class="caption-tl pb-10px">
                    <div class="f-left"><strong>任务评分列表</strong></div>
                </caption>
                <thead>
                <tr class="colhead">
                ...
                <?php if(has_permission($pmsdata['grade']['powers']['gradetasklist']['value'])):?>
                <tbody>
                <?php foreach($data as $key => $val):?>
                    <tr class="a-center odd">
                        <td><?php echo $val->id;?></td>
                        <td title="<?php echo $val->task_name;?>" class="a-left">
                            <a href="/grade/taskview/<?php echo $val->id;?>?body=<?php echo $body;?>"><?php echo $val->task_name;?></a>
                        </td>
                        <td><?php echo empty($val->grade_by)?'':$val->grade_by->real_name;?></td>
                        <td><?php echo $val->is_graded;?></td>
                        <td>
                            <?php if(has_permission($pmsdata['grade']['powers']['taskview']['value'])):?>
                            <a href="/grade/taskview/<?php echo $val->id;?>?body=<?php echo $body;?>"><?php echo $val->is_graded == '未评分'?'评分':'查看';?></a>
                            <?php endif;?>
                        </td>
                    </tr>
                <?php endforeach;?>
