...
<div id="wrap" <?php echo !$include_headfoot ? 'style="padding:0"' : '';?>>
  <div class="outer" <?php echo !$include_headfoot ? 'style="padding:0"' : '';?>>
        <div id="titlebar">
            <div id="main"><?php echo $project_name.' &gt;&gt; TASK #'.$task_id.'&nbsp;'."<a href='/task/view/{$task_id}' target='_blank'>{$task_name}</a>"; ?></div>
        </div>
            ...
            <tr valign="top">
                <td>
                    <div id="comment" class="">
                        <fieldset>
                          <legend>任务描述</legend>
                          <div class="content"><?php echo $task_description;?></div>
                        </fieldset>
                        <fieldset>
                          <legend>附件</legend>
                          <div> 
                            <?php foreach ($task_attachments as $val):?>
                            <a href="/download/<?php echo $val->id;?>"><?php echo empty($val->title) ? basename($val->path) : $val->title.$val->extension;?></a> <br/>
                            <?php endforeach;?>
                          </div>
                        </fieldset>
                        <form id="dataform" method="post" action="/grade/gradetaskedit/<?php echo $grade_id; ?>?body=<?php echo $body;?>">
                            <fieldset>
                                <legend>我的评价</legend>
                                <table align="center" class="table-1 lblock">
                                    ...
                                    <?php foreach($data as $key => $val):?>
                                        ...
                                        <tr>
                                            <th class="rowhead"></th>
                                            <td><textarea name="grade_description[<?php echo $val->id; ?>]" minlength="4" class="area-5" rows="3" <?php echo !empty($val->score)?'disabled':'';?> required><?php if(!empty($val->score)) echo empty($val->score->description)?'':$val->score->description;?></textarea>
                                            </td>
                                        </tr>
                                    <?php endforeach;?>
                                    <?php endif;?>
                                    <tr>
                                        <td class="a-left" colspan="2">
                                            ...
                                            <?php endif;?>
                                            <a class="" href="/grade/gradetasklist/?body=<?php echo $body;?>"><input type="button" value=" 返回 " class="button-r"></a>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
